...
- **Sem distorção por construção:** o SVG mantém a proporção (`xMidYMid meet`) e a altura acompanha a largura (`h-auto`), então linha, nós e textos escalam de forma uniforme — nada de traço achatado nem tipografia esticada. `vector-effect="non-scaling-stroke"` fica como garantia extra de que o traço permanece com 2 px reais em qualquer largura. Se em execução o card exigir altura fixa e o SVG precisar esticar horizontalmente, a alternativa aplicada é `preserveAspectRatio="none"` com `vector-effect="non-scaling-stroke"` em todos os traços e os `<text>` dentro de um `<g>` com contra-escala horizontal medida por `ResizeObserver`.
