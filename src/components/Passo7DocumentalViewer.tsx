...
const PdfAction: React.FC<{
  ...
  if (!url) {
    return (
      <div className="flex min-h-32 flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-line-strong bg-surface-sunken/60 p-5 text-center">
        <span className="flex h-10 w-10 items-center justify-center rounded-lg border border-line-soft bg-surface-raised text-ink-muted">
  ...
  return (
    <div className="space-y-3 rounded-xl border border-line-soft bg-surface-raised p-4 shadow-sm">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0">
          ...
          <div className="truncate text-xs text-ink-muted">{nome || "relatorio.pdf"}</div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2">
        ...
        <a href={url} target="_blank" rel="noopener noreferrer" className={cn(buttonVariants({ variant: "outline", size: "sm" }), "h-9 rounded-lg border-line-strong bg-surface-raised px-3 text-[10px] font-black uppercase text-ink hover:bg-surface-sunken")}>
...
export const Passo7DocumentalViewer: React.FC<Passo7DocumentalViewerProps> = ({
  ...
  return (
    <div className="space-y-5">
      <header className="panel flex items-start gap-3 p-4 sm:p-5">
        ...
        <div className="min-w-0">
          ...
          <h3 className="mt-0.5 font-display text-base font-black text-ink sm:text-lg">
      ...
      {loading ? (
        <div className="panel flex min-h-36 flex-col items-center justify-center gap-2 p-6 text-center text-xs text-ink-muted">
        ...
        <div className="flex min-h-36 flex-col items-center justify-center gap-3 rounded-xl border border-rose-200 bg-rose-50 p-5 text-center text-xs text-rose-700 shadow-sm">
          ...
          <span>{error}</span>
          <Button type="button" variant="outline" size="sm" onClick={onReload} className="rounded-lg border-rose-200 bg-surface-raised text-rose-700 hover:bg-rose-100">
            <RefreshCw className="h-3.5 w-3.5" /> Tentar novamente
          </Button>
        </div>
      ) : consultas.length === 0 ? (
        <div className="flex min-h-36 flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-line-strong bg-surface-sunken/60 p-6 text-center text-xs text-ink-muted">
        ...
        <div className="space-y-4">
          {consultas.map((consulta) => (
            <section key={consulta.id} className="panel overflow-hidden">
              <div className="flex flex-wrap items-start justify-between gap-2 border-b border-line-soft bg-surface-sunken/50 px-4 py-3 sm:px-5">
                <div>
                  ...
                  <h4 className="font-display text-sm font-black text-ink">{consulta.documentoNome || "Consulta de crédito"}</h4>
                  <p className="mt-0.5 font-mono text-[10px] text-ink-muted">{consulta.documento}</p>
                </div>
                <span className="font-mono text-[10px] text-ink-muted">{consulta.dataConsulta ? new Date(consulta.dataConsulta).toLocaleString("pt-BR") : ""}</span>
              </div>

              <div className={`grid gap-4 p-4 sm:p-5 ${isApto ? "grid-cols-1" : "lg:grid-cols-2"}`}>
                <div className="space-y-2">
                  <div className="flex items-center gap-2 font-mono text-[10px] font-black uppercase tracking-wider text-ink-muted">
                    <span className="h-1.5 w-1.5 rounded-full bg-ink-muted" /> {isApto ? "Relatório Inicial" : "Antes"}
                  </div>
